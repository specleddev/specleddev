...
4. Run the guided reconcile step. In `specled_ex`, that is `mix spec.assist --bugfix`.
5. If the current subject already states the intended behavior, keep the test and tighten a scenario only when the wording is ambiguous.
6. If the intended behavior is missing, update the requirement or scenario before merge.
7. Update docs if the behavior is user-facing.
8. Run the strict check before merge.
