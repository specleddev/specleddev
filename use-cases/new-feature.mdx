...
5. Run `npx specled verify`, then `npx specled check`, and fix drift before merge.

## Codex example prompt

```text
Implement this feature strictly from the .spec/specs/ folder.
Update tests and docs to match the new requirements and scenarios.
Run npx specled verify and resolve all warnings.
...
Run npx specled verify and return unresolved drift findings.
```
...
- Confirm npx specled check passes.
